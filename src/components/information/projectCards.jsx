...
function ProjectCard({ cardsInfo }) {
  ...
  return (
    <div
      ref={ref}
      className={`cards-base group bg-comic-background bg-cover bg-[color:var(--tertiary-color)] w-[380px] h-[210px] sm:w-[550px] sm:h-[300px] shadow-md border-2 border-black flex gap-4 rounded-md transition-all ease-in-out ${
        inView ? "opacity-100 translate-x-0" : "opacity-0 translate-x-[100%]"
      }`}
    >
      <div
        className={`cards-base-left w-[40%] py-4 pl-4 flex-col transition-all ease-in-out delay-500 ${
          inView ? "opacity-100 translate-x-0" : "opacity-0 translate-x-[-100%]"
        }`}
      >
      ...
      <div
        className={`cards-base-right w-[60%] flex justify-center py-4 pr-4 transition-all ease-in-out delay-[600ms] ${
          inView ? "opacity-100 translate-x-0" : "opacity-0 translate-x-[-100%]"
        }`}
      >
        <img
        ...
        <img
          ...
          alt=""
          className='z-10 h-[70px] w-[70px] absolute bottom-[-6%] sm:bottom-[-4%] left-[2%]'
        />
        ...
        <a
          ...
          className='z-10 absolute h-[40px] w-[40px] bottom-[2%] m-2 left-[16%] opacity-0 scale-0 transition-all ease-in-out duration-700 group-hover:left-[40%] group-hover:opacity-100 group-hover:scale-90'
        >
...
const ProjectCards = () => {
  return (
    <div className='grid h-full xl:grid-cols-2 lg:grid-cols-1 md:grid-cols-1 gap-12 p-2'>
      {projectInfo.map((cardsInfo) => (
        <ProjectCard key={cardsInfo.id} cardsInfo={cardsInfo} />
      ))}
    </div>
  );
};
